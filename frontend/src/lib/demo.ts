...
function buildEnhancement(component: string): EnhancementPlan {
  ...
  return {
    component,
    owned,
    summary: `${component} → ${targets} is missing structured logs on the policy-decision path, so its decisions never reach Splunk. Add MDC trace-id logging and a latency alert.`,
    rationale: [
